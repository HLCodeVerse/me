      ...
      header={
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <h1 style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em' }}>Analytics & Insights</h1>
          </div>
          <button
      ...
      <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {aiReport && (
          <div className="card" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, color: '#10B981', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            ...
            { label: 'Task Completion', value: taskStats.rate,             unit: '%',   icon: Target,     color: '#8B5CF6' },
            { label: 'Journal Entries', value: journalCount,               unit: '',    icon: BookOpen,   color: '#06B6D4' },
            { label: 'Current Streak',  value: profile?.current_streak ?? 0, unit: ' days', icon: Zap,    color: '#F59E0B' },
          ].map(({ label, value, unit, icon: Icon, color }) => (
            <div key={label} className="card" style={{ padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: 'var(--text-dim)', fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{label}</span>
        ...
        <div className="card" style={{ padding: '18px 16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 26, fontWeight: 800, color: '#8B5CF6' }}>{taskStats.completed}</span>
            <div>
        ...
        <div className="card" style={{ padding: '18px 16px' }}>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <LineChart data={data}>
              <XAxis dataKey="date" tick={{ fill: 'var(--text-dim)', fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip {...TOOLTIP_STYLE} />
              <Line type="monotone" dataKey="mood" stroke="#06B6D4" strokeWidth={2.5} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
                ...
                <div key={s.type} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 12, color: '#F59E0B', fontWeight: 700 }}>{s.current}d streak</span>
                    </div>
                    <div className="progress-track">
                      <div className="progress-fill" style={{ width: `${s.longest > 0 ? (s.current / s.longest) * 100 : 0}%`, background: '#F59E0B' }} />
                    </div>
